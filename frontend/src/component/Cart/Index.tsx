...
import { fetchProducts } from '@/redux/authSlice';

const Cart = () => {
    const dispatch = useDispatch();

    const { cart, totalQuantity, totalAmount, prod, status, error } = useSelector((state: any) => state.auth)

    useEffect(() => {
        dispatch(fetchProducts() as any);
    }, []);

    const items: any = useSelector((state: any) => state?.auth?.product)
    ...
    const pickup = Math.round(totalAmount >= 2000 ? 99 : 199 || 0)
    // const tax = Math.round(totalAmount >= 4000 ? 99 : 199)

    const calculateTax = (totalAmount: any) => {
        let taxRate;

        if (totalAmount < 1000) {
            taxRate = 0.05; // 5% tax for amounts less than 1000
        } else if (totalAmount >= 1000 && totalAmount < 4000) {
            taxRate = 0.1; // 10% tax for amounts between 1000 and 4000
        } else {
            taxRate = 0.15; // 15% tax for amounts 4000 and above
        }
                ...
                <div className="mx-auto max-w-screen-xl px-4 2xl:px-0">
                    ...
                    <div className="mt-6 sm:mt-8 md:gap-6 lg:flex lg:items-start xl:gap-8">
                        <div className="mx-auto w-full flex-none lg:max-w-2xl xl:max-w-4xl">
                                        ...
                                        <div className="space-y-4 md:flex md:items-center md:justify-between md:gap-6 md:space-y-0">
                                            ...
                                            <>
                                                <div className="w-full min-w-0 flex-1 space-y-4 md:order-2 md:max-w-md">
                                                    ...
                                                    <Link href="#" className="text-base font-medium text-gray-900 hover:underline dark:text-white">{item.name}</Link>
                                                ...
                                                <div className="text-end md:order-4 md:w-32">
                                                    ...
                                                    <p className="text-base font-bold text-gray-900 dark:text-white">Total.

                                                        {item.quantity}
                                                    </p>
                                                    <p className="text-base font-bold text-gray-900 dark:text-white">${item.price}</p>

                                                    <p className="text-base font-bold text-gray-900 dark:text-white">Total - ${item.price * item.quantity}</p>
                                                </div>
                                            </>
                                        </div>
                                    </div>
                            ...
                            <div className=" xl:mt-8 xl:block flex">
                                ...
                                <div className="mt-6 grid grid-cols-3 gap-4 sm:mt-8">
                                    {items.slice(0, 3).map((item: any) => (
                                        <div className="space-y-6 overflow-hidden rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                            ...
                                            />
                                            <div>
                                                <Link href="#" className="text-lg font-semibold leading-tight text-gray-900 hover:underline dark:text-white">{item.name}</Link>
                                                <p className="mt-2 text-base font-normal text-gray-500 dark:text-gray-400">{item.desc}</p>
                                            </div>
                                            <div>
                                                <p className="text-lg font-bold text-gray-900 dark:text-white">
                                                    <span className="text-[#BB5A3A]"> ${item.price} </span>
                                                </p>
                                ...
                                <div className="space-y-4">
                                    <div className="space-y-2">
                                        ...
                                        <dl className="flex items-center justify-between gap-4">
                                            ...
                                            <dd className="text-base font-medium text-gray-900 dark:text-white">${pickup}
                                            </dd>
                                        </dl>
                                    ...
                                    <dl className="flex items-center justify-between gap-4 border-t border-gray-200 pt-2 dark:border-gray-700">
                                        ...
                                        <dd className="text-base font-bold text-gray-900 dark:text-white">
                                            ${(totalAmount >= 5000 ? totalAmount * 0.95 : totalAmount * 1 + pickup + calculateTax(totalAmount)).toFixed(2)}
                                        </dd>
                                    </dl>

                                </div>
                                <div>
                                    <Link href="/cart/checkout" className='bg-[#BB5A3A] p-3 text-white cursor-pointer mt-4 mb-4 w-full inline-flex items-center justify-center'>Checkout</Link>
                                </div>

                                <div className="flex items-center justify-center gap-2">
                                    <span className="text-sm font-normal text-gray-500 dark:text-gray-400"> or </span>
                                    <Link href="/home" title="" className="inline-flex items-center gap-2 text-sm font-medium text-primary-700 underline hover:no-underline dark:text-primary-500">
                                        Continue Shopping
                                        <svg className="h-5 w-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
